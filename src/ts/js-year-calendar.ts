...
export default class Calendar<T extends CalendarDataSourceElement> {
  ...
  constructor(
    ...
    options: CalendarOptions<T> = null
  ) {
  ...
  protected _initializeOptions(opt: any): void {
    ...
    this.options = {
      ...
      enableRangeSelection:
        opt.enableRangeSelection != null ? opt.enableRangeSelection : false,
      disabledDays: opt.disabledDays instanceof Array ? opt.disabledDays : [],
      disabledWeekDays:
        opt.disabledWeekDays instanceof Array ? opt.disabledWeekDays : [],
      hiddenWeekDays:
        opt.hiddenWeekDays instanceof Array ? opt.hiddenWeekDays : [],
      roundRangeLimits:
        opt.roundRangeLimits != null ? opt.roundRangeLimits : false,
      dataSource:
        opt.dataSource instanceof Array || typeof opt.dataSource === 'function'
          ? opt.dataSource
          : [],
      style:
      ...
      contextMenuItems:
        opt.contextMenuItems instanceof Array ? opt.contextMenuItems : [],
      customDayRenderer:
  ...
  public render(isLoading: boolean = false): void {
    ...
    if (isLoading) {
      ...
      // Fade animation
      var months = this.element.querySelector(
        '.months-container'
      ) as HTMLElement;
  ...
  protected _renderHeader(): void {
    ...
    if (this._isFullYearMode()) {
      ...
      if (
        ...
        this.options.maxDate < new Date(this._startDate.getFullYear() + 1, 0, 1)
      ) {
      ...
      if (
        this.options.maxDate != null &&
        this.options.maxDate < new Date(this._startDate.getFullYear() + 2, 0, 1)
      ) {
    ...
    if (this._dataSource != null && this._dataSource.length > 0) {
      this.element
        ...
        .forEach((month: HTMLElement) => {
          ...
          if (
            (this.options.minDate == null || lastDate > this.options.minDate) &&
            ...
            for (var i = 0; i < this._dataSource.length; i++) {
              if (
                !(this._dataSource[i].startDate >= lastDate) ||
            ...
            if (monthData.length > 0) {
              month
                ...
                .forEach((day: HTMLElement) => {
                  ...
                  if (
                    (this.options.minDate == null ||
                      currentDate >= this.options.minDate) &&
                    (this.options.maxDate == null ||
                      currentDate <= this.options.maxDate)
                  ) {
                    ...
                    if (
                      dayData.length > 0 &&
                      (this.options.displayDisabledDataSource ||
  ...
  protected _renderDataSourceDay(
    elt: HTMLElement,
    currentDate: Date,
    events: T[]
  ): void {
      ...
      case 'border':
        ...
        if (weight > 0) {
          ...
          for (var i = 0; i < events.length; i++) {
            ...
            boxShadow += `inset 0 -${(i + 1) * weight}px 0 0 ${
              events[i].color
            }`;
          }
        ...
        .forEach((element) => {
          element.addEventListener('click', (e) => {
            if (
              ...
              this.setYear(
                parseInt((e.currentTarget as HTMLElement).textContent)
              );
            }
          });
        });
                ...
                this.setStartDate(
                  new Date(
                    ...
                    this._startDate.getMonth() -
                      this.options.numberMonthsDisplayed,
                    1
                  )
                );
              }, 50);
        ...
        cell.addEventListener('mousedown', (e: MouseEvent) => {
          if (e.which == 1) {
            ...
            if (
              ...
              this.isThereFreeSlot(currentDate)
            ) {
  ...
  protected _computeMonthsSize(): void {
    ...
    var monthSize =
      (this.element.querySelector('.month') as HTMLElement).offsetWidth + 10;
    this._nbCols = null;

    if (
      monthSize * 6 < calendarSize &&
      this.options.numberMonthsDisplayed >= 6
    ) {
      this._nbCols = 2;
    } else if (
      monthSize * 4 < calendarSize &&
      this.options.numberMonthsDisplayed >= 4
    ) {
      this._nbCols = 3;
    } else if (
      monthSize * 3 < calendarSize &&
      this.options.numberMonthsDisplayed >= 3
    ) {
      this._nbCols = 4;
    } else if (
      monthSize * 2 < calendarSize &&
      this.options.numberMonthsDisplayed >= 2
    ) {
      this._nbCols = 6;
    } else {
      this._nbCols = 12;
    ...
    this.element.querySelectorAll('.month-container').forEach((month) => {
      if (!month.classList.contains(`month-${this._nbCols}`)) {
        ['month-2', 'month-3', 'month-4', 'month-6', 'month-12'].forEach(
          (className) => {
  ...
  protected _refreshRange(): void {
    this.element
      ...
      .forEach((day) => day.classList.remove('range'));
    this.element
    ...
    if (this._mouseDown) {
      var minDate =
        this._rangeStart < this._rangeEnd ? this._rangeStart : this._rangeEnd;
      var maxDate =
        this._rangeEnd > this._rangeStart ? this._rangeEnd : this._rangeStart;
      ...
      this.element
        ...
        .forEach((month: HTMLElement) => {
          ...
          const monthEndDate = new Date(
            this._startDate.getFullYear(),
            this._startDate.getMonth() + monthId + 1,
            1
          );
  ...
  protected _openContextMenu(elt: HTMLElement): void {
    ...
    if (contextMenu.children.length > 0) {
      ...
      if (
        contextMenu.getBoundingClientRect().right > document.body.offsetWidth
      ) {
      ...
      const closeContextMenu = (event: Event) => {
        if (
          event.type !== 'click' ||
          !contextMenu.contains((event as MouseEvent).target as Node)
        ) {
          contextMenu.style.display = 'none';
  ...
  protected _renderContextMenuItems(
    parent: HTMLElement,
      ...
      if (items[i].click) {
        (function (index) {
          menuItemContent.addEventListener('click', () => {
            (
              document.querySelector('.calendar-context-menu') as HTMLElement
            ).style.display = 'none';
  ...
  public isThereFreeSlot(date: Date, after: Boolean = null): Boolean {
    ...
    if (after === true) {
      return !events.some(
        (evt) =>
          (!this.options.alwaysHalfDay && !evt.endHalfDay) || evt.endDate > date
      );
    } else if (after === false) {
      return !events.some(
        (evt) =>
          (!this.options.alwaysHalfDay && !evt.startHalfDay) ||
          evt.startDate < date
      );
    } else {
      return (
        this.isThereFreeSlot(date, false) || this.isThereFreeSlot(date, true)
      );
    }
  }
  ...
  public setStartDate(startDate: Date): void {
    if (startDate instanceof Date) {
      ...
      const periodEventResult = this._triggerEvent('periodChanged', {
   ...
   */
  public setNumberMonthsDisplayed(
    numberMonthsDisplayed: number | string,
    preventRendering: boolean = false
  ): void {
   ...
   * @param preventRedering Indicates whether the rendering should be prevented after the property update.
   */
  public setDisplayDisabledDataSource(
   ...
   * @param preventRedering Indicates whether the rendering should be prevented after the property update.
   */
  public setEnableRangeSelection(
   ...
   */
  public getCustomDataSourceRenderer(): (
    element: HTMLElement,
    currentDate: Date,
    events: T[]
  ) => void {
   ...
   * @param preventRedering Indicates whether the rendering should be prevented after the property update.
   */
  public setCustomDataSourceRenderer(
    customDataSourceRenderer: (
      element: HTMLElement,
      currentDate: Date,
      events: T[]
  ...
  public setWeekStart(
    ...
    this.options.weekStart = !isNaN(parseInt(weekStart as string))
      ? parseInt(weekStart as string)
      : null;
